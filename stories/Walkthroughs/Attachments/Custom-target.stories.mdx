...
The Upload function of the `Attachments` component can be mapped to different components.
...
const CustomAttachmentsDemo = () => {
  ...
  return (
    <>
      ...
      <Attachments className="pt-2" isIndependent={false} ref={attachmentsRef} attachments={attachments} onChange={setAttachments}/>
    </>
  );
};
