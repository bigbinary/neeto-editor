...
export const isEditorOverlaysActive = () => {
  const isTippyContentActive = !isEmpty(
    document.getElementsByClassName("tippy-content")
  );

  const isOverlaysActive = !isEmpty(
    document.getElementsByClassName("ne-media-uploader")
  );

  const isEmbedModalOpen = !isEmpty(
    document.getElementsByClassName("ne-embed-modal")
  );

  return isTippyContentActive || isOverlaysActive || isEmbedModalOpen;
};
