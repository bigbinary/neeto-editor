const applyMarkdownWordFormatting = (inputFieldRef, decorator = "") => {
...
const applyMarkdownLineFormatting = (
  inputFieldRef,
  decorator,
  isOrderedList = false
) => {
  ...
  lastLine = lastLine === -1 ? textArea.value.length : lastLine;
...
export const handleKeyDown = ({
  ...
  if (length >= limit && event.key !== "Backspace") event.preventDefault();
    ...
    applyMarkdownLineFormatting(inputFieldRef, "> ");
  else if (event.key === "b" && (event.metaKey || event.ctrlKey))
    applyMarkdownWordFormatting(inputFieldRef, "**");
  else if (event.key === "i" && (event.metaKey || event.ctrlKey))
    applyMarkdownWordFormatting(inputFieldRef, "_");
    ...
    applyMarkdownLineFormatting(inputFieldRef, null, true);
  else if (
    event.key === "8" &&
    (event.metaKey || event.ctrlKey) &&
    event.shiftKey
