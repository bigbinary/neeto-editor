...
const useMarkdownEditor = ({
  ...
  // subscribe to onFocus and onBlur handlers
  useEffect(() => {
    if (!inputFieldRef.current) return;
    inputFieldRef.current.addEventListener("focus", handleFocus);
    inputFieldRef.current.addEventListener("blur", handleBlur);
    return () => {
      if (!inputFieldRef.current) return;
      inputFieldRef.current.removeEventListener("focus", handleFocus);
      inputFieldRef.current.removeEventListener("blur", handleBlur);
    };
  }, [inputFieldRef.current, handleFocus, handleBlur]);
