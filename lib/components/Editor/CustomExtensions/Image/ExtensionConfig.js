...
const upload = file =>
  new Promise((resolve, reject) => {
    const uploader = new DirectUpload(file, DIRECT_UPLOAD_ENDPOINT);

    uploader.create((error, { signed_id }) => {
      if (error) return reject(error);
      const imageUrl = `${window.location.origin}/rails/active_storage/blobs/redirect/${signed_id}/${file.name}`;
      return resolve(imageUrl);
    });
  });

export default {
  configure: ({ uploadEndpoint = DIRECT_UPLOAD_ENDPOINT }) =>
