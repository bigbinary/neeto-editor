import React, { useState } from "react";
import classnames from "classnames";
import { BubbleMenu } from "@tiptap/react";
import { roundArrow } from "tippy.js";
import "tippy.js/dist/svg-arrow.css";
import Modal from "components/Common/Modal";
import ToolTip from "components/Common/ToolTip";
import { capitalize } from "utils/common";

import { getImageMenuOptions, getTextMenuOptions } from "./constants";
import LinkOption from "./LinkOption";

import ImageEditor from "../Image/ImageEditor";

export default function index({ editor, options }) {
  const [isLinkOptionActive, setIsLinkOptionActive] = useState(false);
  const [isInvalidLink, setIsInvalidLink] = useState(false);
  const [isImageEditorModalOpen, setIsImageEditorModalOpen] = useState(false);
  const node = editor && editor.view.state.selection.node;
  const isImage = node && node.type.name === "image";

  if (!editor) {
    return null;
  }

  const handleAnimateInvalidLink = () => {
    setIsInvalidLink(true);
    const timer = setTimeout(() => {
      setIsInvalidLink(false);
      clearTimeout(timer);
    }, 1000);
  };

  let bubbleMenuOptions = null;
  if (isImage) {
    bubbleMenuOptions = getImageMenuOptions({
      editor,
      isImageEditorModalOpen,
      setIsImageEditorModalOpen,
    }).map((option) => <Option key={option.optionName} {...option} />);
  } else if (isLinkOptionActive) {
    bubbleMenuOptions = (
      <LinkOption
        editor={editor}
        handleClose={() => setIsLinkOptionActive(false)}
        handleAnimateInvalidLink={handleAnimateInvalidLink}
      />
    );
  } else {
    bubbleMenuOptions = getTextMenuOptions({
      editor,
      setIsLinkOptionActive,
    })
      .filter(({ optionName }) => options.includes(optionName))
      .map((option) => <Option {...option} key={option.optionName} />);
  }

  return (
    <div>
      <BubbleMenu
        editor={editor}
        tippyOptions={{
          arrow: roundArrow,
          placement: "top-start",
          zIndex: 99999,
          onHide: () => setIsLinkOptionActive(false),
        }}
        className={classnames("neeto-editor-bubble-menu", {
          "neeto-editor-bubble-menu-animate-shake": isInvalidLink,
        })}
      >
        {bubbleMenuOptions}
      </BubbleMenu>
      <Modal
        isOpen={isImageEditorModalOpen}
        onClose={() => setIsImageEditorModalOpen(false)}
      >
        <div className="neeto-editor-image-uploader">
          <div className="neeto-editor-image-uploader__content">
            <ImageEditor
              editor={editor}
              onClose={() => setIsImageEditorModalOpen(false)}
              url={node?.attrs.src}
              alt={node?.attrs.alt}
            />
          </div>
        </div>
      </Modal>
    </div>
  );
}

const Option = ({ Icon, command, active, optionName }) => (
  <ToolTip content={capitalize(optionName)} position="top" delay={[500]}>
    <div
      className={classnames("neeto-editor-bubble-menu__item", {
        active: active,
      })}
      onClick={command}
      data-cy={`neeto-editor-bubble-menu-${optionName}-option`}
    >
      <Icon size={20} />
    </div>
  </ToolTip>
);
